...
| **История диалога** | Хранится в памяти процесса — всё «как сейчас» в коде. | По умолчанию **теряется между вызовами** (каждый запрос может обрабатываться новым экземпляром). Чтобы сохранять историю, нужна внешняя БД (YDB, Redis и т.п.). |
