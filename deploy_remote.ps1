...
$remoteCmd = "cd ~/tg-ai-enhel-method && git pull && venv/bin/pip install -r requirements.txt && sudo systemctl restart tg-ai-enhel-method && echo '' && echo '--- Service status ---' && sudo systemctl status tg-ai-enhel-method --no-pager"
