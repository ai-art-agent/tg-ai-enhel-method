...
### A.1. Узнать chat_id чата, куда слать уведомления

1. Создайте группу в Telegram или выберите личный чат (например, «Избранное» с ботом).
2. Добавьте в группу бота (того же, чей токен в `TELEGRAM_BOT_TOKEN`), если это группа.
3. Узнайте chat_id:
   - Напишите в этот чат любое сообщение.
   - Откройте в браузере:  
     `https://api.telegram.org/bot<ВАШ_ТОКЕН>/getUpdates`  
     (подставьте вместо `<ВАШ_ТОКЕН>` значение из `TELEGRAM_BOT_TOKEN`).
   - В ответе найдите `"chat":{"id": -123456789}` или `"chat":{"id": 123456789}`. Число `id` — это chat_id (у групп оно обычно отрицательное).
...
Убедитесь, что есть строка (если нет — добавьте, подставьте свой chat_id):
